...
#              退避轮询 (MIN_WAIT_SEC 起步，最长 MAX_WAIT_SEC)，满员等待时
#              提示已提交进度 (最近提交的文件夹与作业 ID)，有空位立即
#              快速补位。
...
JOB_SCRIPT="sub*.sh"   # 各文件夹内的作业提交脚本 (GPUMD: sub_MD.sh；VASP: sub2.sh)，目录内只能存在一个匹配脚本
...
while [ $index -lt $total ]; do
    ...
    while [ $submitted -lt $can_submit ] && [ $index -lt $total ]; do
        ...
        if [ -z "$jobid" ]; then
            ...
            # 当作成功直接丢弃，导致 90 个目录被静默跳过)；日志中显式留痕
