...
帧记录与去重:
  每次成功抽取的帧会追加写入当前运行目录下的 extracted_frames.txt，
  每行包含: 帧号、输出绝对路径、事件、状态。
...
def update_record_status():
    ...
    for line in lines:
        ...
        lines_out.append(f"{idx:<22}{path:<{path_col}}{event:<16}{status}\n")
    with open(RECORD_FILE, "w", encoding="utf-8") as f:
        f.writelines(lines_out)
    return n_exist, n_lost


def append_record(indices, outdir, recorded):
    """将新抽取的帧追加到 RECORD_FILE，带事件与状态列:
    ...
    with open(RECORD_FILE, "a", encoding="utf-8") as f:
        ...
        for idx in indices:
            ...
            f.write(f"{idx:<22}{out_path:<{path_col}}{event:<16}存在\n")
...
def main():
    ...
    if rest[0] == "random":
        ...
        append_record(picked, outdir, recorded)
    else:
        # 固定抽帧模式 (默认)
        ovito_indices = []
        seen = set()
        ...
        if not to_extract:
            ...
            for idx in to_extract:
                extract_and_save(frames, idx, outdir)
            append_record(to_extract, outdir, recorded)
