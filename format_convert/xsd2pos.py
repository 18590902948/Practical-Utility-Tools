...
脚本:        xsd2pos.py
分类:        格式转换脚本
功能:        将 Materials Studio 导出的 XSD 结构文件转换为 VASP POSCAR；
             支持固定原子（Selective dynamics 标记）与 P1 对称性处理；
             输出文件为 .vasp 格式。
使用方法:    python xsd2pos.py [file.xsd]
参数:        file.xsd    可选，指定要转换的 XSD 文件；
                         不带参数时自动批量转换当前目录下全部 *.xsd 文件
输出:
  xxx.vasp             VASP 结构文件，direct 坐标
作者:        Hongbo Sun
最后修改日期: 2026-08-20
=============================================================================
# 目录树示例:
# ============================================================================
# .                       # 指定文件: python xsd2pos.py A.xsd
# ├── A.xsd              # 输入：Materials Studio 结构文件
# └── A.vasp             # 输出：VASP 结构文件
# .                       # 批量模式: python xsd2pos.py（转换全部 .xsd）
# ├── A.xsd
# ├── A.vasp
# ├── B.xsd
# └── B.vasp
...
class VASP(object):
    ...
    def xyz_read(self,filename):
        ...
        #fix bug no xyz if atom in 0,0,0
    ...
    if len(sys.argv)==1:
        ...
        if len(glob("*.xsd"))==0:
            ...
            for i in glob("*.xsd"):
                poscar=VASP()
                poscar.xyz_read(i)
                poscar.xyz_write(i.split(".")[0]+".vasp")
        ...
        if os.path.exists(sys.argv[1]):
            ...
            poscar.xyz_write(xsdname.split(".")[0]+".vasp")
        else:
