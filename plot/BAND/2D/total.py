...
# DOS相关文件
FILE_TDOS = os.path.join(DATA_DIR, "TDOS.dat")  # 总态密度
FILE_PDOS_AL = os.path.join(DATA_DIR, "PDOS_Al.dat")  # Al分波态密度
FILE_PDOS_O = os.path.join(DATA_DIR, "PDOS_O.dat")  # O分波态密度

# ---------------------- 3. 文本显示参数（空字符串则不显示） ----------------------
TITLE_TEXT = ''  # 图表总标题
BAND_Y_LABEL = 'Energy (eV)'  # 能带Y轴标签
# DOS标签
TDOS_X_LABEL = 'TDOS'  # 总态密度X轴标签
PDOS_AL_X_LABEL = 'PDOS-Al'  # Al分波态密度X轴标签
PDOS_O_X_LABEL = 'PDOS-O'  # O分波态密度X轴标签

# ---------------------- 4. 位置控制参数 ----------------------
# 标签距离控制
BAND_Y_LABEL_PAD = 10  # 能带Y轴标签距离Y轴的距离
# DOS位置参数
TDOS_X_LABEL_PAD = 10  # TDOS X轴标签距离X轴的距离
PDOS_AL_X_LABEL_PAD = 10  # Al PDOS X轴标签距离X轴的距离
PDOS_O_X_LABEL_PAD = 10  # O PDOS X轴标签距离X轴的距离
TITLE_PAD = 12  # 标题距离图表上边框的距离
# 刻度距离控制
BAND_X_TICK_PAD = 8  # 能带X轴刻度标签距离X轴的距离
BAND_Y_TICK_PAD = 8  # 能带Y轴刻度标签距离Y轴的距离
DOS_X_TICK_PAD = 8  # DOS X轴刻度标签距离X轴的距离
DOS_Y_TICK_PAD = 8  # DOS Y轴刻度标签距离Y轴的距离

# ---------------------- 5. 视觉样式 - 颜色 ----------------------
COLOR_BAND = 'blue'  # 能带曲线颜色
COLOR_TDOS = 'blue'  # 总态密度曲线颜色
# Al PDOS颜色
COLOR_PDOS_AL_S = 'black'
COLOR_PDOS_AL_PX = 'red'
COLOR_PDOS_AL_PY = 'yellow'
COLOR_PDOS_AL_PZ = 'green'
# O PDOS颜色
COLOR_PDOS_O_S = 'black'
COLOR_PDOS_O_PX = 'red'
COLOR_PDOS_O_PY = 'yellow'
COLOR_PDOS_O_PZ = 'green'
# 其他颜色
...
LINEWIDTH_BASE_TDOS = 2.5  # TDOS曲线线宽基准
LINEWIDTH_BASE_PDOS = 2.5  # PDOS曲线线宽基准
LINEWIDTH_BASE_SPINE = 2.5  # 图表边框线宽基准
# 刻度线宽
LINEWIDTH_BASE_BAND_TICK_MAJOR = 2.5  # 能带主刻度线宽基准
LINEWIDTH_BASE_DOS_TICK_MAJOR = 2.5  # DOS主刻度线宽基准
# 辅助线线宽
...
# DOS字体大小
FONTSIZE_BASE_TDOS_X_LABEL = 28  # TDOS X轴标签字体大小基准
FONTSIZE_BASE_PDOS_AL_X_LABEL = 28  # Al PDOS X轴标签字体大小基准
FONTSIZE_BASE_PDOS_O_X_LABEL = 28  # O PDOS X轴标签字体大小基准
FONTSIZE_BASE_LEGEND = 24  # 图例字体大小基准
FONTSIZE_BASE_ERROR_TEXT = 26  # 数据缺失提示文字大小基准

# ---------------------- 8. 视觉样式 - 刻度长度基准值 ----------------------
TICKLENGTH_BASE_BAND_MAJOR = 8.0  # 能带主刻度长度基准
TICKLENGTH_BASE_DOS_MAJOR = 8.0  # DOS主刻度长度基准

# ---------------------- 9. 视觉样式 - 坐标轴范围（None=自动计算） ----------------------
Y_RANGE = [-10,10]  # 能量轴范围 [最小值, 最大值]（偏移后自动计算）
# DOS X轴范围
TDOS_X_RANGE = None  # TDOS X轴范围
PDOS_AL_X_RANGE = None  # Al PDOS X轴范围
PDOS_O_X_RANGE = None  # O PDOS X轴范围
Y_TICK_DECIMAL = 0  # 能量轴刻度小数位数

# ---------------------- 10. 视觉样式 - 刻度配置 ----------------------
TICK_DIRECTION = 'in'  # 刻度方向：'in'/'out'/'inout'
DOS_X_TICK_DECIMAL = 0  # DOS X轴刻度小数位数
Y_MAJOR_TICK = 2  # 能量轴主刻度间隔（eV）- 现在会生效
DOS_X_MAJOR_TICK = None  # DOS X轴主刻度间隔
# DOS刻度显示控制
DOS_X_TICK_LABEL_SHOW = False  # 是否显示DOS X轴刻度标签
DOS_Y_TICK_LABEL_SHOW = False  # 是否显示DOS Y轴刻度标签
...
# ---------------------- 12. 视觉样式 - 图例 ----------------------
LEGEND_SHOW = True  # O PDOS图例显示开关
LEGEND_LOC = 'upper right'  # 图例位置
LEGEND_FRAME_ON = False  # 图例边框开关
LEGEND_FRAME_ALPHA = 1.0  # 图例边框透明度
LEGEND_LABELS = ['s', 'p$_x$', 'p$_y$', 'p$_z$']  # O PDOS图例标签
...
# ---------------------- 14. 显示开关（核心配置） ----------------------
SHOW_VBM_LINE = False  # 是否显示VBM虚线
SHOW_CBM_LINE = False  # 是否显示CBM虚线
SHOW_VBM_MARKER = False  # 是否显示VBM标记点
SHOW_CBM_MARKER = False  # 是否显示CBM标记点
...
# ---------------------- 16. 画布&布局配置（新增统一区域间距参数） ----------------------
FIGSIZE = (15, 8)  # 总画布尺寸（宽15=6+3+3+3，高8）
GRID_WIDTH_RATIOS = [6, 3, 3, 3]  # 子图宽度比例（能带:TDOS:Al PDOS:O PDOS）
INTER_REGION_SPACING = 0.05  # 统一控制区域间间距：能带-TDOS、TDOS-PDOS-Al、PDOS-Al-PDOS-O
...
FINAL_LINEWIDTH_TDOS = LINEWIDTH_BASE_TDOS * GLOBAL_LINEWIDTH_SCALE
FINAL_LINEWIDTH_PDOS = LINEWIDTH_BASE_PDOS * GLOBAL_LINEWIDTH_SCALE
...
FINAL_LINEWIDTH_DOS_TICK_MAJOR = LINEWIDTH_BASE_DOS_TICK_MAJOR * GLOBAL_LINEWIDTH_SCALE

# 刻度最终长度
FINAL_TICKLENGTH_BAND_MAJOR = TICKLENGTH_BASE_BAND_MAJOR * GLOBAL_LINEWIDTH_SCALE
FINAL_TICKLENGTH_DOS_MAJOR = TICKLENGTH_BASE_DOS_MAJOR * GLOBAL_LINEWIDTH_SCALE
...
FINAL_FONTSIZE_TDOS_X_LABEL = FONTSIZE_BASE_TDOS_X_LABEL * GLOBAL_FONT_SIZE_SCALE
FINAL_FONTSIZE_PDOS_AL_X_LABEL = FONTSIZE_BASE_PDOS_AL_X_LABEL * GLOBAL_FONT_SIZE_SCALE
FINAL_FONTSIZE_PDOS_O_X_LABEL = FONTSIZE_BASE_PDOS_O_X_LABEL * GLOBAL_FONT_SIZE_SCALE
...
# ---------------------- DOS工具函数 ----------------------
def get_tick_kwargs(axis: str, is_dos: bool = False) -> Dict[str, Any]:
    """生成刻度配置参数"""
    kwargs = {'direction': TICK_DIRECTION}
    # 刻度标签距离
    if axis == 'x':
        kwargs['pad'] = BAND_X_TICK_PAD if not is_dos else DOS_X_TICK_PAD
    else:
        kwargs['pad'] = BAND_Y_TICK_PAD if not is_dos else DOS_Y_TICK_PAD
    # 字体大小
    if not is_dos:
        kwargs['labelsize'] = FINAL_FONTSIZE_BAND_X_TICK if axis == 'x' else FINAL_FONTSIZE_BAND_Y_TICK
    else:
        kwargs['labelsize'] = None
    return kwargs


def read_dos_data() -> Tuple[
    np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """读取TDOS和PDOS数据"""
    # 总态密度
    tdos_energy, tdos_dos = np.array([]), np.array([])
    try:
        tdos_data = np.loadtxt(FILE_TDOS, skiprows=1)
        tdos_energy, tdos_dos = tdos_data[:, 0], tdos_data[:, 1]
        print(f"✅ 读取TDOS：{len(tdos_energy)}个能量点")
    except Exception as e:
        print(f"⚠️  TDOS数据读取失败：{str(e)}")

    # Al PDOS
    al_energy, al_s, al_px, al_py, al_pz = np.array([]), np.array([]), np.array([]), np.array([]), np.array([])
    try:
        al_data = np.loadtxt(FILE_PDOS_AL, skiprows=1)
        al_energy = al_data[:, 0]
        al_s = al_data[:, 1]  # s轨道
        al_px = al_data[:, 4]  # px轨道
        al_py = al_data[:, 2]  # py轨道
        al_pz = al_data[:, 3]  # pz轨道
        print(f"✅ 读取PDOS-Al：{len(al_energy)}个能量点")
    except Exception as e:
        print(f"⚠️  Al PDOS数据读取失败：{str(e)}")

    # O PDOS
    o_energy, o_s, o_px, o_py, o_pz = np.array([]), np.array([]), np.array([]), np.array([]), np.array([])
    try:
        o_data = np.loadtxt(FILE_PDOS_O, skiprows=1)
        o_energy = o_data[:, 0]
        o_s = o_data[:, 1]  # s轨道
        o_px = o_data[:, 4]  # px轨道
        o_py = o_data[:, 2]  # py轨道
        o_pz = o_data[:, 3]  # pz轨道
        print(f"✅ 读取PDOS-O：{len(o_energy)}个能量点")
    except Exception as e:
        print(f"⚠️  O PDOS数据读取失败：{str(e)}")

    return tdos_energy, tdos_dos, al_energy, al_s, al_px, al_py, al_pz, o_energy, o_s, o_px, o_py, o_pz
...
def apply_energy_offset(
        ...
        tdos_energy: np.ndarray,
        al_energy: np.ndarray,
        o_energy: np.ndarray
) -> Tuple[float, float, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """应用能量偏移，让0eV对齐VBM和CBM的中心"""
    if not OFFSET_TO_VBM_CBM_CENTER or np.isnan(vbm_energy) or np.isnan(cbm_energy):
        return vbm_energy, cbm_energy, band_energies, tdos_energy, al_energy, o_energy
    ...
    tdos_energy_offset = tdos_energy + offset if tdos_energy.size > 0 else tdos_energy
    al_energy_offset = al_energy + offset if al_energy.size > 0 else al_energy
    o_energy_offset = o_energy + offset if o_energy.size > 0 else o_energy

    print(f"✅ 偏移后：VBM={vbm_energy_offset:.4f} eV，CBM={cbm_energy_offset:.4f} eV")
    return vbm_energy_offset, cbm_energy_offset, band_energies_offset, tdos_energy_offset, al_energy_offset, o_energy_offset
...
# ---------------------- DOS绘图函数 ----------------------
def plot_tdos(ax: plt.Axes, tdos_energy: np.ndarray, tdos_dos: np.ndarray) -> None:
    """绘制总态密度图"""
    if len(tdos_energy) == 0 or len(tdos_dos) == 0:
        ax.text(0.5, 0.5, 'TDOS Data Missing', ha='center', va='center',
                fontsize=FINAL_FONTSIZE_ERROR_TEXT, color='red')
        return

    # 绘制TDOS曲线
    ax.plot(tdos_dos, tdos_energy, color=COLOR_TDOS, linewidth=FINAL_LINEWIDTH_TDOS)

    # 费米能级线
    if SHOW_FERMI_LINE:
        ax.axhline(y=FERMI_LEVEL_Y, color=COLOR_FERMI_LEVEL, linestyle=FERMI_LINE_LINestyle,
                   alpha=VBM_CBM_ALPHA, linewidth=FINAL_LINEWIDTH_FERMI)

    # 坐标轴设置
    ax.set_xlabel(TDOS_X_LABEL, fontsize=FINAL_FONTSIZE_TDOS_X_LABEL, labelpad=TDOS_X_LABEL_PAD)
    if TDOS_X_RANGE is not None:
        ax.set_xlim(TDOS_X_RANGE)

    # DOS刻度样式
    ax.tick_params(axis='x', which='major', length=FINAL_TICKLENGTH_DOS_MAJOR,
                   width=FINAL_LINEWIDTH_DOS_TICK_MAJOR, **get_tick_kwargs('x', is_dos=True))
    ax.tick_params(axis='y', which='major', length=FINAL_TICKLENGTH_DOS_MAJOR,
                   width=FINAL_LINEWIDTH_DOS_TICK_MAJOR, **get_tick_kwargs('y', is_dos=True))

    # 刻度标签显示控制
    ax.tick_params(axis='x', labelbottom=DOS_X_TICK_LABEL_SHOW)
    ax.tick_params(axis='y', labelleft=DOS_Y_TICK_LABEL_SHOW)
...
def plot_al_pdos(ax: plt.Axes, al_energy: np.ndarray, al_s: np.ndarray, al_px: np.ndarray,
                 al_py: np.ndarray, al_pz: np.ndarray) -> None:
    """绘制Al分波态密度图"""
    if len(al_energy) == 0:
        ax.text(0.5, 0.5, 'Al PDOS Missing', ha='center', va='center',
                fontsize=FINAL_FONTSIZE_ERROR_TEXT, color='red')
        return

    # 绘制Al PDOS各轨道曲线
    ax.plot(al_s, al_energy, color=COLOR_PDOS_AL_S, linewidth=FINAL_LINEWIDTH_PDOS)
    ax.plot(al_px, al_energy, color=COLOR_PDOS_AL_PX, linewidth=FINAL_LINEWIDTH_PDOS)
    ax.plot(al_py, al_energy, color=COLOR_PDOS_AL_PY, linewidth=FINAL_LINEWIDTH_PDOS)
    ax.plot(al_pz, al_energy, color=COLOR_PDOS_AL_PZ, linewidth=FINAL_LINEWIDTH_PDOS)
    ...
    # 坐标轴设置
    ax.set_xlabel(PDOS_AL_X_LABEL, fontsize=FINAL_FONTSIZE_PDOS_AL_X_LABEL, labelpad=PDOS_AL_X_LABEL_PAD)
    if PDOS_AL_X_RANGE is not None:
        ax.set_xlim(PDOS_AL_X_RANGE)
...
def plot_o_pdos(ax: plt.Axes, o_energy: np.ndarray, o_s: np.ndarray, o_px: np.ndarray,
                o_py: np.ndarray, o_pz: np.ndarray) -> None:
    """绘制O分波态密度图"""
    if len(o_energy) == 0:
        ax.text(0.5, 0.5, 'O PDOS Missing', ha='center', va='center',
                fontsize=FINAL_FONTSIZE_ERROR_TEXT, color='red')
        return

    # 绘制O PDOS各轨道曲线
    lines = []
    lines.append(ax.plot(o_s, o_energy, color=COLOR_PDOS_O_S, linewidth=FINAL_LINEWIDTH_PDOS)[0])
    lines.append(ax.plot(o_px, o_energy, color=COLOR_PDOS_O_PX, linewidth=FINAL_LINEWIDTH_PDOS)[0])
    lines.append(ax.plot(o_py, o_energy, color=COLOR_PDOS_O_PY, linewidth=FINAL_LINEWIDTH_PDOS)[0])
    lines.append(ax.plot(o_pz, o_energy, color=COLOR_PDOS_O_PZ, linewidth=FINAL_LINEWIDTH_PDOS)[0])
    ...
    # 坐标轴设置
    ax.set_xlabel(PDOS_O_X_LABEL, fontsize=FINAL_FONTSIZE_PDOS_O_X_LABEL, labelpad=PDOS_O_X_LABEL_PAD)
    if PDOS_O_X_RANGE is not None:
        ax.set_xlim(PDOS_O_X_RANGE)
    ...
    # 图例
    if LEGEND_SHOW:
        legend_kwargs = {'fontsize': FINAL_FONTSIZE_LEGEND, 'loc': LEGEND_LOC, 'frameon': LEGEND_FRAME_ON}
        if LEGEND_FRAME_ON:
            legend_kwargs['framealpha'] = LEGEND_FRAME_ALPHA
        legend = ax.legend(lines, LEGEND_LABELS, **legend_kwargs)
        if LEGEND_FRAME_ON:
...
# ============================ 核心绘图函数 =============================
# ==============================================================================
def plot_band_dos_structure():
    """绘制能带（6宽）+ TDOS/PDOS（各3宽）的完整图"""
    # 1. 加载所有数据
    ...
    # 读取DOS数据
    tdos_energy, tdos_dos, al_energy, al_s, al_px, al_py, al_pz, o_energy, o_s, o_px, o_py, o_pz = read_dos_data()
    ...
    # 4. 应用能量偏移（新增步骤）
    vbm_energy_offset, cbm_energy_offset, band_energies_offset, tdos_energy_offset, al_energy_offset, o_energy_offset = apply_energy_offset(
        vbm_energy, cbm_energy, band_energies, tdos_energy, al_energy, o_energy
    )
    ...
    if len(tdos_energy_offset) > 0:
        energy_ranges.extend([tdos_energy_offset.min(), tdos_energy_offset.max()])
    if len(al_energy_offset) > 0:
        energy_ranges.extend([al_energy_offset.min(), al_energy_offset.max()])
    if len(o_energy_offset) > 0:
        energy_ranges.extend([o_energy_offset.min(), o_energy_offset.max()])

    y_min, y_max = (-10, 10)
    if energy_ranges:
        y_min, y_max = (min(energy_ranges), max(energy_ranges)) if Y_RANGE is None else Y_RANGE

    # 6. 创建画布&布局（用新增参数控制区域间间距）
    fig = plt.figure(figsize=FIGSIZE)
    gs = GridSpec(1, 4, width_ratios=GRID_WIDTH_RATIOS, wspace=INTER_REGION_SPACING)
    ...
    # 8. 绘制能带窗口（占第一个区域，宽6）
    band_gs = gs[0].subgridspec(1, band_window_count, wspace=WINDOW_SPACING)
    # 先创建第一个能带窗口（作为Y轴基准）
    band_axs = [fig.add_subplot(band_gs[0])]
    # 后续能带窗口共享第一个的Y轴
    for i in range(1, band_window_count):
        band_axs.append(fig.add_subplot(band_gs[i], sharey=band_axs[0]))
    ...
    # 9. 绘制DOS（占后三个区域，各宽3），共享最左侧能带窗口的Y轴（使用偏移后的能量数据）
    # TDOS
    ax_tdos = fig.add_subplot(gs[1], sharey=band_axs[0])
    plot_tdos(ax_tdos, tdos_energy_offset, tdos_dos)
    ax_tdos.set_ylim(y_min, y_max)
    ax_tdos.tick_params(axis='y', labelleft=False)  # 隐藏DOS的Y轴刻度标签

    # PDOS-Al
    ax_al = fig.add_subplot(gs[2], sharey=band_axs[0])
    plot_al_pdos(ax_al, al_energy_offset, al_s, al_px, al_py, al_pz)
    ax_al.set_ylim(y_min, y_max)
    ax_al.tick_params(axis='y', labelleft=False)  # 隐藏DOS的Y轴刻度标签

    # PDOS-O
    ax_o = fig.add_subplot(gs[3], sharey=band_axs[0])
    plot_o_pdos(ax_o, o_energy_offset, o_s, o_px, o_py, o_pz)
    ax_o.set_ylim(y_min, y_max)
    ax_o.tick_params(axis='y', labelleft=False)  # 隐藏DOS的Y轴刻度标签

    # 统一Y轴刻度：应用2.0eV间隔（核心修复）
    y_major_locator = MultipleLocator(Y_MAJOR_TICK)
    for ax in band_axs + [ax_tdos, ax_al, ax_o]:
        ax.yaxis.set_major_locator(y_major_locator)  # 强制Y轴主刻度间隔为2.0eV
        ax.yaxis.set_major_formatter(FormatStrFormatter(f"%.{Y_TICK_DECIMAL}f"))
...
if __name__ == "__main__":
    print("\n" + "=" * 70)
    print("📌 能带+DOS绘图配置（统一控制区域间距）")
    print("=" * 70)
    print(f"总画布：宽{FIGSIZE[0]} × 高{FIGSIZE[1]}")
    print(f"区域比例：能带(6) | TDOS(3) | PDOS-Al(3) | PDOS-O(3)")
    print(f"区域间统一间距：{INTER_REGION_SPACING}（控制能带-TDOS、TDOS-PDOS-Al、PDOS-Al-PDOS-O）")
    print(f"Y轴配置：仅最左侧能带窗口显示数值刻度 | 所有子图共用同一Y轴")
    print(f"Y轴主刻度间隔：{Y_MAJOR_TICK} eV（已生效）")
    print(f"全局线宽倍率：{GLOBAL_LINEWIDTH_SCALE} | 全局字体倍率：{GLOBAL_FONT_SIZE_SCALE}")
    print(f"VBM/CBM中心对齐开关：{'开启' if OFFSET_TO_VBM_CBM_CENTER else '关闭'}")  # 修正变量名拼写
    if OFFSET_TO_VBM_CBM_CENTER:
        print("⚠️  开启后0eV将对齐VBM和CBM的中心，所有能量数据将自动偏移")
    print("=" * 70 + "\n")

    try:
        plot_band_dos_structure()
    except Exception as e:
        print(f"\n❌ 绘图失败：{str(e)}")