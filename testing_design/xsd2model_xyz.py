...
脚本:        xsd2model_xyz.py
分类:        格式转换脚本
功能:        将 Materials Studio 导出的 XSD 结构文件批量转换为 model.xyz
             (标准 extxyz 格式，Lattice/Properties/pbc 属性行)，每个输入
             按文件名建立独立子文件夹输出 (如 1.xsd -> 1/model.xyz)，
             与 LiH_xyz 编号文件夹结构一致；元素按周期表顺序排序
             (Mo 在 Te 之前)；记录文件审计每次转换。
使用方法:    python xsd2model_xyz.py [输入xsd文件 ...] [-o 输出目录] [-h]
参数:        输入xsd文件   输入 XSD 文件 (可多个，支持通配符；不指定时用配置区
                         INPUT_FILES；先按当前运行目录探测，不存在再相对脚本目录)
             -o/--output  输出根目录 (默认: 配置区 OUTPUT_PATH)，每个输入
                         在其下建立 <basename>/ 子文件夹并输出 model.xyz
             -h/--help   显示本帮助
输入文件:    配置区 INPUT_FILES (默认 *.xsd，支持通配符)
输出文件:
  <输出根目录>/<basename>/model.xyz   每个输入一个子文件夹 (如 1.xsd -> 1/model.xyz)
  xsd2model_xyz.txt       记录文件 (表格形式，表头#开头+5列，每次运行追加)
输出路径:    默认脚本所在目录 (OUTPUT_PATH)，可用 -o 指定 (相对/绝对路径均可)
示例:
  python xsd2model_xyz.py ./1.xsd ./2.xsd ./3.xsd
  python xsd2model_xyz.py '*.xsd' -o ./LiH_xyz
  python xsd2model_xyz.py ./H.xsd -o ./out
作者:        隼蝶.
最后修改日期: 2026-08-27
=============================================================================
# 目录树示例:
# ============================================================================
# .                       # 运行: python xsd2model_xyz.py 1.xsd 2.xsd -o ./LiH_xyz
# ├── 1.xsd              # 输入：Materials Studio 结构文件
# ├── 2.xsd              # 输入：Materials Studio 结构文件
# ├── LiH_xyz/           # 输出根目录 (-o 指定)
# │   ├── 1/             # 输出子文件夹 (按输入 basename 命名)
# │   │   └── model.xyz  # 输出：extxyz 结构文件
# │   └── 2/             # 输出子文件夹 (按输入 basename 命名)
# │       └── model.xyz  # 输出：extxyz 结构文件
# └── xsd2model_xyz.txt  # 记录文件 (转换清单)
# ============================================================================
"""
...
OUTPUT_NAME  = "model.xyz"              # 输出文件名 (每个输入在 <basename>/ 子文件夹下输出该文件名)
RECORD_FILE  = "xsd2model_xyz.txt"      # 转换记录文件 (脚本所在目录，表格形式，每次运行追加)
INPUT_PATH   = "./"                     # 输入文件寻找路径 (相对脚本所在目录)
OUTPUT_PATH  = "./"                     # 输出根目录 (相对脚本所在目录；每个输入在其下建立 <basename>/ 子文件夹)
...
def parse_args(argv):
    ...
    -o/--output 指定输出根目录。返回 (输入文件列表, 输出目录或 None)。"""
    inputs = []
    output = None
    i = 0
    while i < len(argv):
...
def convert_xsd_to_model(xsd_path, out_dir):
    """转换单个 XSD 文件为 model.xyz 并写入 <out_dir>/<basename>/model.xyz
    (覆盖模式，自动创建子文件夹)。返回 (原子数, 输出文件绝对路径)。"""
    ...
    subdir = os.path.join(out_dir,
                          os.path.splitext(os.path.basename(xsd_path))[0])
    os.makedirs(subdir, exist_ok=True)
    out_path = os.path.join(subdir, OUTPUT_NAME)
    write(out_path, atoms, format="extxyz")
    return len(atoms), os.path.abspath(out_path)
...
def main():
    ...
    if output is not None:
        outdir = resolve_path(script_dir, output)
    else:
        outdir = os.path.normpath(os.path.join(script_dir, OUTPUT_PATH))
    os.makedirs(outdir, exist_ok=True)
    print(f"ℹ️ 输出根目录: {os.path.abspath(outdir)}")
    ...
    for i, src in enumerate(inputs, 1):
        ...
        try:
            ...
            natom, dst = convert_xsd_to_model(src, outdir)
            event = "覆盖" if existed else "新建"
            n_ok += 1
            print(f"  ✅ [{i}/{len(inputs)}] {os.path.basename(src)} "
                  f"-> {dst} ({natom} 原子)")
            records.append((os.path.abspath(src), natom, dst, event, "成功"))
