...
脚本:        vasp2cif.py
分类:        格式转换脚本
功能:        将 VASP 格式文件 (*.vasp / POSCAR* / CONTCAR*) 转换为标准 CIF
             晶体学信息文件 (晶格与原子分数坐标完整保留，多帧写入多个
             data_ 块)；-o 指定目录时按文件类型分类输出 vasp.cif / pos.cif /
             cont.cif，-o 指定文件时全部合并为一个文件；记录文件审计
             每次转换。
使用方法:    python vasp2cif.py [输入文件 ...] [-o 输出] [-h]
参数:        输入文件    VASP 格式输入文件 (*.vasp / POSCAR* / CONTCAR*)，
                        支持通配符与跨目录路径，可多个；不指定时自动扫描
                        脚本所在目录下全部 VASP 文件
                        (位置参数前可加 -t/--target 标记，仅为声明、可省略)
             -o/--output 输出文件或输出目录：以 .cif 结尾视为输出
                        文件完整路径 (全部输入合并为一个文件)；否则视为
                        输出目录 (按文件类型分类输出 vasp.cif / pos.cif /
                        cont.cif)；默认输出到脚本所在目录 (OUTPUT_PATH)
             -h/--help   显示本帮助
输入文件:    *.vasp / POSCAR* / CONTCAR* (VASP 格式)
输出文件:    vasp2cif.txt  (记录文件，表格形式，表头#+多列，覆盖模式，
                        与输出文件同目录)
             <输出目录>/vasp.cif    由 *.vasp 文件转换合并
             <输出目录>/pos.cif     由 POSCAR* 文件转换合并
             <输出目录>/cont.cif    由 CONTCAR* 文件转换合并
             (或 -o 指定的单个合并文件)
输出路径:    默认脚本所在目录 (OUTPUT_PATH)，可用 -o 指定；相对路径遵循
             "带 ./ 相对当前运行目录、不带点相对脚本目录" 规则
示例:
  python vasp2cif.py ./A.vasp ./B.vasp ./POSCAR ./CONTCAR -o ./out/
  python vasp2cif.py './POSCAR*' './CONTCAR*' -o ./pos.cif
  python vasp2cif.py ./A.vasp ./B.vasp -o ./D.cif
  python vasp2cif.py                     (扫描脚本目录全部 VASP 文件)
# 目录树示例:
# ============================================================================
# .                        # 脚本所在目录 (含 VASP 格式文件)
# ├── 1.vasp               # 输入: VASP 文件
...
# ├── vasp.cif             # 输出: 由 *.vasp 转换合并
# ├── pos.cif              # 输出: 由 POSCAR* 转换合并
# ├── cont.cif             # 输出: 由 CONTCAR* 转换合并
# └── vasp2cif.txt         # 输出: 转换记录文件
# ============================================================================
作者:        隼蝶.
最后修改日期: 2026-08-27
=============================================================================
...
OUTPUT_FILES = ["vasp.cif", "pos.cif", "cont.cif"]  # 分类输出文件名 (对应 vasp/poscar/contcar 三组)
RECORD_FILE  = "vasp2cif.txt"      # 转换记录文件 (与输出文件同目录)
...
def resolve_output(script_dir, output):
    """解析输出: -o 以 .cif 结尾视为输出文件完整路径 (全部输入合并为一个
    文件)，否则视为输出目录 (按类型分类输出)；省略 -o 时用配置区
    OUTPUT_PATH。返回 (输出目录绝对路径, 合并输出文件路径或 None)。"""
    if output:
        path = resolve_cmd_path(script_dir, output)
        if output.lower().endswith(".cif"):
...
def classify_vasp_file(name):
    ...
    CONTCAR* -> contcar；非 VASP 文件 (含 cif 输出文件) 返回 None。"""
    lower = name.lower()
    if lower.endswith((".cif", ".xyz", ".extxyz")):
        return None  # 排除输出文件，避免把上次输出当输入
...
def write_group(frames, out_path):
    """将一组帧写入 CIF 文件 (覆盖模式，多帧写为多个 data_imageN 块，
    P1 全原子分数坐标，ASE 3.28 write_cif 行为)。返回帧数。"""
    if not frames:
        return 0
    write(out_path, frames, format="cif")
    return len(frames)
...
def main():
    ...
    if n_total == 0:
        print("❌ 错误: 未找到任何 VASP 格式输入文件 "
              "(*.vasp / POSCAR* / CONTCAR*)。")
        print("用法: python vasp2cif.py [输入文件 ...] -o 输出")
        sys.exit(1)
    ...
    if out_file:
        ...
        n = write_group(frames_all, out_file)
        if n:
        ...
        for gname, outname in zip(("vasp", "poscar", "contcar"), OUTPUT_FILES):
            ...
            n = write_group(frames_g, out_path)
            if n:
